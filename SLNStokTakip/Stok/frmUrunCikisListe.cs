    ...
    public partial class frmUrunCikisListe : Form
    {
        ...
        void Listele()
        {
            ...
            var srg = (from s in _db.stUrunCikis
                       where s.bgFirma.Fadi.Contains(txtCikisListesi.Text)
                       select new
                       {
                           ckod=s.CikisNo,
                           fad=s.bgFirma.Fadi,
        ...
        void Sec()
        {
            try
            {
                if (Liste.CurrentRow != null) _secimId = int.Parse(Liste.CurrentRow.Cells[0].Value.ToString());// Çıkış no yu _secimId ye atadık.
            }
            catch (Exception)
            {
                _secimId = -1;
        ...
        private void txtCikisListesi_TextChanged(object sender, EventArgs e)
        {
            Listele();
        }
