...
namespace SLNStokTakip.Stok
{
    public partial class frmUrunKayitListe : Form
    {
        ...
        void Listele()
        {
            ...
            var lst = (from s in _db.stUrunKayitUsts
                       ...
                       select new
                       {
                           ...
                           gt = s.GirisTarih,
                           fn = s.FaturaNo,
                       }).OrderByDescending(x => x.gt).ThenBy(y => y.fa);//ilk tarih sonra firma adlarını a-z sıralıyor.
        ...
        void Sec()
        {
            try
            {
                if (Liste.CurrentRow != null) _secimId = int.Parse(Liste.CurrentRow.Cells[0].Value.ToString());// Giriş no yu _secimId ye atadık.
            }
            catch (Exception)
            {
                _secimId = -1;
